    ...
    >
      <div className='flex items-center h-full justify-between w-[92%] mx-auto '>
            ...
            {NavLinks.map((link) => {
              return (
                <Link
                  key={link.id}
                  href={link.url}
                  className='text-base hover:text-cyan-700 font-medium transition-all duration-200 space-x-2'>
                  <p>{link.label} </p>
                </Link>
              )
            })}
        ...
        <div className='flex items-center space-x-4'>
          ...
          <button className='px-8 py-2.5 text-xs sm:text-sm rounded-lg cursor-pointer bg-gray-100 hover:bg-gray-300 transition-all duration-300'>Login/register
          </button>
          {/* Job Post Button */}
          <button className='px-8 py-2.5 text-sm hidden sm:block cursor-pointer rounded-lg bg-cyan-700 hover:bg-cyan-900 transition-all duration-300 text-white'>Job Post
          </button>
          {/* Theme Toggler */}
